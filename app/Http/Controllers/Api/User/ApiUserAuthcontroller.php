...
class ApiUserAuthcontroller extends UserApiBaseController
{
        ...
        if (!empty($postdata))
        {
            if ($this->request->username && $this->request->password)
            {
                ...
                {				
					...
					$op['is_merchant'] = 0;	
					if ($res['accinfo']->account_type == 3) {
						if (!empty($res['accinfo']->is_multi_store)) {
							/* for multistore need to hide the store code */
							//$op['store_code'] = '';
							$op['store_code'] = $res['accinfo']->store_code;
							$op['supplier_id'] = $res['accinfo']->supplier_id; 
						} else { 
							$op['store_code'] = $res['accinfo']->store_code; 
							$op['supplier_id'] = $res['accinfo']->supplier_id; 
						}
						$op['is_multi_store'] = $res['accinfo']->is_multi_store; 
						$op['is_merchant'] = 1; 
					} else if ($res['accinfo']->account_type == 5) {
						$op['store_code'] = $res['accinfo']->store_code; 
						$op['supplier_id'] = $res['accinfo']->supplier_id; 
						$op['is_merchant'] = 1;
					}
	...
	public function merchant_stores_list() 
	{				
		if (($this->userSess->account_type == $this->config->get('constants.ACCOUNT_TYPE.SELLER'))) {
			$stores = $this->userSess->stores;
			array_walk($stores, function($result) 
            {
				$result->logo = isset($result->logo) ? $result->logo : asset(Config::get('path.SELLER.STORE_IMG_PATH.WEB').$result->store_logo);
				unset($result->store_id, $result->primary_store, $result->supplier_id, $result->store_logo);
			});
			$op['stores'] = $stores;
			$op['status'] = $this->statusCode = $this->config->get('httperr.SUCCESS');				
		} else {			
			$op['status'] = $this->statusCode = $this->config->get('httperr.UN_PROCESSABLE');
			$op['msg'] = 'Stores Not Found';
		}
		return $this->response->json($op, $this->statusCode, $this->headers, $this->options);
	}
    ...
    {	
        ...
        if (!empty($postdata))
        {
            $res = $this->accountObj->instore_details($postdata);	
			
            if ($res)
            {
                $this->userSess->store_code = $res->store_code;
                $this->userSess->store_name = $res->store_name;
                $this->userSess->store_info = (object) [
							'store_code'=>$res->store_code,	
							'supplier_id'=>$res->supplier_id,	
							'store_id'=>$res->store_id,	
							'store_name'=>$res->store_name,	
							'store_mobile'=>$res->mobile,	
                ...
                $op['data'] = $this->userSess->store_info;
                $op['msg'] = 'Store Updated Successfully';
