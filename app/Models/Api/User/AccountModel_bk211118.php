...
class AccountModel extends BaseModel
{
        ...
        if (!empty($email) || !empty($mobile) || !empty($unique_id) || !empty($account_id))
        {
            ...
            { 
		        if($userData->account_type_id == $this->config->get('constants.ACCOUNT_TYPE.SELLER') || $userData->account_type_id == $this->config->get('constants.ACCOUNT_TYPE.SELLER_EMPLOYEE')) {
					...
					/* $adqry =DB::table($this->config->get('tables.SUPPLIER_MST').' as sm')
							->join($this->config->get('tables.ADDRESS_MST') . ' as addm', function($join)
							{
								$join->on('addm.relative_post_id', '=', 'sm.supplier_id')
									->where('addm.post_type', '=', $this->config->get('constants.ADDRESS_POST_TYPE.SELLER'))
									->where('addm.address_type_id', '=', $this->config->get('constants.ADDRESS.PRIMARY'));
							})
							->where('sm.account_id', '=', $userData->account_id); */
							//return $userData->account_type_id;
					$adqry = DB::table($this->config->get('tables.STORE_EMPLOYEES').' as semp')
							...
							{										
								if ($userData->account_type_id === 3) {
									$join->on('addm.relative_post_id', '=', 'semp.supplier_id')
										 ->where('addm.post_type', '=', $this->config->get('constants.ADDRESS_POST_TYPE.SELLER'))
										 ->where('addm.address_type_id', '=', $this->config->get('constants.ADDRESS.PRIMARY'));
								}
    ...
    {  
		...
		if (in_array($account_id, [142, 116, 113])) {
		    $query->whereDate('mo.order_date', '>=', date('Y-m-d'));
        }
