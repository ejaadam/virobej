...
class AccountModel extends BaseModel
{
            ...
            { 
				if ($userData->account_type_id == $this->config->get('constants.ACCOUNT_TYPE.SELLER')) {
					$store = DB::table($this->config->get('tables.SUPPLIER_MST').' as sm')
									 ...
									 ->Join($this->config->get('tables.STORES_EXTRAS').' as se', 'se.store_id', '=', 's.store_id')									
									 ...
									 ->selectRAW('s.store_code, s.store_name, s.primary_store, s.store_logo, s.store_id, sm.supplier_id, se.mobile_no as mobile, se.email');
		        ...
		        if($userData->account_type_id == $this->config->get('constants.ACCOUNT_TYPE.SELLER') || $userData->account_type_id == $this->config->get('constants.ACCOUNT_TYPE.SELLER_EMPLOYEE')) {
					...
					if (($userData->account_type_id == $this->config->get('constants.ACCOUNT_TYPE.SELLER_EMPLOYEE'))) {		
						 $adqry = DB::table($this->config->get('tables.STORE_EMPLOYEES').' as semp')
								...
								->selectRAW('s.store_code, s.supplier_id, s.store_name, se.mobile_no as mobile, se.email')  
				...
				$adqry->leftJoin($this->config->get('tables.LOCATION_PINCODES').' as lp', 'lp.pincode', '=', 'addm.postal_code')
					  ...
					  ->leftJoin($this->config->get('tables.LOCATION_COUNTRY').' as lc', 'lc.country_id', '=', 'addm.country_id')
					  ->addselect('addm.state_id', 'addm.district_id', 'addm.city_id as locality_id', 'addm.geolat', 'addm.geolng', 'addm.flatno_street', 'addm.address', 'addm.landmark', 'addm.postal_code', 'lp.pincode_id', 'ls.region_id', 'ls.state', 'ld.district', 'lcty.city', 'lc.country');
					...
					{						
						...
						{								 
							...
							if (($userData->account_type_id == 3) || ($userData->account_type_id == 5))
							{
								...
								//if (!empty($is_multi_store)) { $userAddr->store_code = ''; }
								$data = array_merge($data, [
									'store_code'=>isset($userAddr->store_code) ? (string) $userAddr->store_code : '',
									'store_id'=>!empty($userAddr->store_id) ? (string) $userAddr->store_id : $stores[0]->store_id,	
									...
									'store_info'=>(object) [
										'store_code'=>!empty($userAddr->store_code) ? $userAddr->store_code : $stores[0]->store_code,	
										'supplier_id'=>!empty($userAddr->supplier_id) ? $userAddr->supplier_id : $stores[0]->supplier_id,	
										'store_name'=>isset($userAddr->store_name) ? $userAddr->store_name : $stores[0]->store_name,	
										'store_mobile'=>isset($userAddr->mobile) ? $userAddr->mobile : $stores[0]->mobile,	
										'store_email'=>isset($userAddr->email) ? $userAddr->email : $stores[0]->email,	
										'lat'=>$userAddr->geolat,
										'lng'=>$userAddr->geolng,
										'address'=>[
											'location'=>$userAddr->address,
											'landmark'=>$userAddr->landmark,
											'city'=>$userAddr->city,
    ...
    {	//return $arr;		
        ...
        $qry->selectRaw("sm.supplier_id, sm.category_id, sm.company_name as business_name, sm.logo as mrlogo, st.store_name, st.store_code, st.store_logo as logo, st.store_id, bcl.bcategory_name, am.address as location, am.landmark, am.geolat as lat, am.geolng as lng, CONCAT_WS(' ',lc.phonecode,se.mobile_no) as mobile, scs.redeem as accept_pay, lcty.city, lc.country, se.email, am.postal_code");	
	...
	public function getOrderInfo($order_code){
		return DB::table($this->config->get('tables.ORDERS').' as mrd')
                        ...
                        ->selectRaw('mrd.order_id, sm.store_id, sm.supplier_id as mrid, mm.company_name as mrbusiness_name, mm.supplier_code as mrcode, se.mobile_no as mobile, sm.store_code, sm.store_name, mrd.account_id')
                        ->first();
	}
