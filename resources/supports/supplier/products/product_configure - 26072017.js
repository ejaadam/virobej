...
$.fn.extend({
    productConfigure: function (options) {
        ...
        $.ajax({
            data: _this.options,
            url: window.location.API.SUPPLIER + 'products/configure',
...
function updateProductDetails(Pro) {
    ...
    if (Pro.product_details.is_combinations) {
        ...
        $('#add_stock .panel-body #combination_id').loadSelect({
            url: window.location.API.SUPPLIER + 'products/combinations/select-list',
            key: 'product_cmb_id',
    ...
    $('#product-price').on('click', '.delete-product-price', function (e) {
        ...
        if (confirm('Are you sure, You wants to delete ' + CurEle.data('currency'))) {
            $.ajax({
                url: window.location.API.SUPPLIER + 'products/price/delete',
    ...
    $('#shipment-form').validate({
        ...
        submitHandler: function (form, event) {
            ...
            $.ajax({
                data: $(form).serialize(),
                url: window.location.BASE + 'supplier/products/zones/save',
                success: function (data) {
    ...
    PCF.on('click', '#product_comb', function (e) {
        ...
        $.ajax({
            ...
            url: window.location.API.SUPPLIER + 'products/combinations/save',
            beforeSend: function () {
                $('.alert').remove();
            },
            success: function (data) {
    ...
    $('#extraupload').uploadFile({
        url: window.location.API.SUPPLIER + 'products/image/add',
        method: 'POST',
        fileName: 'file',
        dragDrop: false,
        multiple: true,
    ...
    /*    $('#product_properties_form').on('submit', function (e) {
     ...
     if (checked === true) {
     $.ajax({
     data: form.serialize(), url: window.location.API.SUPPLIER + 'products/properties/save',
     success: function (data) {
     }
     });
     }
...
//manage image related functions
    ...
    function print_table(data) {
        ...
        $.each(data, function (key, elm) {
            ...
            if (elm.primary == 1)
            {
                content += '<td> <input type="radio" class="set_default hidden"  name="cover_image" checked="checked" value="' + elm.id + '" data-url ="supplier/products/cover_images"> <span data-img_id ="' + elm.id + '" class="col-sm-8 pull-right icon-edit hidden"></span><button data-img_id ="' + elm.id + '" class="btn btn-small remove pull-right"> <i class="icon-remove"></i></button><span class="pull-right mR10"> <button class="btn btn-small up"><i class="fa fa-arrow-up"></i></button><button class="btn btn-small down"><i class="fa fa-arrow-down"></i></button></span></td></tr>';
            }
            else
            {
                content += '<td> <input type="radio" class="set_default hidden"  name="cover_image" value="' + elm.id + '" data-url ="supplier/products/cover_images"><span data-img_id ="' + elm.id + '" class="col-sm-8 pull-right icon-edit hidden"></span> <button data-img_id ="' + elm.id + '" class="btn btn-small remove pull-right"> <i class="icon-remove"></i></button><span class="pull-right mR10"><button class="btn btn-small up"><i class="fa fa-arrow-up"></i></button><button class="btn btn-small down"><i class="fa fa-arrow-down"></i></button></span></td></tr>';
            }
        ...
        $.each(data, function (key, elm) {
            $('#gallery_grid').append($('<li>', {class: 'mix user_0 business travel mix_all', style: 'display: inline-block; opacity: 1;'}).append([
                $('<a>', {class: 'img_wrapper gal_lightbox', href: 'javascript:void(0);'}).append([
                    $('<img>', {class: 'img img-responsive img-thumbnail', src: elm.img_path}),
    ...
    CP.on('click', '#make_as_def', function (e) {
        ...
        if (confirm('Are you sure Want to make this image as Default ')) {
            $.ajax({
                url: window.location.API.SUPPLIER + 'products/image/make-default',
    ...
    CP.on('click', '#add_selected_image', function (e) {
        ...
        if (data != '' && combination != '') {
            $.ajax({
                ...
                url: window.location.API.SUPPLIER + 'supplier/products/image/combination/add',
                success: function (data) {
                    updateImgPos();
    ...
    CP.on('click', '#delete_selected_image', function (e) {
        ...
        if (data != '' && combination != '')
        {
            ...
            $.ajax({
                data: ajax_data,
                url: window.location.API.SUPPLIER + 'products/image/delete-selected',
                success: function (data) {
    ...
    CP.on('click', '#combination_table #edit_combination_product', function (e) {
        ...
        $('.panel-body #property_id', PCF).loadSelect({
            url: window.location.API.SUPPLIER + 'products/combinations/properties-list',
            key: 'property_id',
            value: 'property',
            data: {product_id: Pro.product_id},
            success: function () {
                ...
                $('.panel-body #value_id', PCF).loadSelect({
                    url: window.location.API.SUPPLIER + 'products/combinations/values-list',
                    key: 'value_id',
                    value: 'value',
                    data: {product_id: Pro.product_id},
        ...
        $.ajax({
            ...
            url: window.location.API.SUPPLIER + 'products/combinations/details',
            success: function (data) {
    ...
    CP.on('click', '#add-supplier-product-combination-btn', function (e) {
        ...
        $('#product_cmb_id', SPCF).loadSelect({
            url: window.location.API.SUPPLIER + 'products/combinations/list',
            key: 'product_cmb_id',
            value: 'product_cmb',
    ...
    CP.on('submit', SPCF, function (e) {
        ...
        $.ajax({
            data: form.serialize(),
            url: window.location.API.SUPPLIER + 'products/save',
            success: function (data) {
                $('#add_combination').hide();
                $('#combination_list').show();
                CDT.fnDraw();
    ...
    CP.on('click', '#add_stock', function (e) {
        ...
        $.ajax({
            ...
            url: window.location.API.SUPPLIER + 'products/update-stock',
            beforeSend: function () {
                $('.alert').remove();
            },
            success: function (data) {
    ...
    CP.on('click', '.remove', function (event) {
        ...
        if (confirm('Are you sure Want to delete this Product Image')) {
            $.ajax({
                ...
                url: window.location.API.SUPPLIER + 'products/image/delete',
                beforeSend: function () {
                    $('#image_values').empty();
                },
                success: function (data) {
    ...
    CP.on('click', '#save_changes', function () {
        var comb_id = $('#combination_select').val();
        $.ajax({
            url: window.location.API.SUPPLIER + 'products/update-sort-order',
