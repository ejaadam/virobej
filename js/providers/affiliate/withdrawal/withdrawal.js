$(document).ready(function () { 
    ...
    $.ajax({
        url: PT.data('url'), 
        success: function (data) { //alert(PT.data('url'));return false;
            PT.append(function () { 
                var types=[]; 
                $.each(data.withdrawal, function (k, e) { 
                    types.push($('<a>', {class: 'btn col-sm-12 btn-default payouts', 'data-payout_type_key': e.payout_type_key, }).append([ 
    ...
    PT.on('click', '.payouts', function (e) {
        e.preventDefault();
        var CurEle=$(this); 
        $.ajax({
            url: F.data('url'),
			type: 'POST',
            data: {payout_type_key: CurEle.data('payout_type_key')},
    ...
    function updateWithdrawForm(data) { 
        if (data.payout_type_details!=undefined) { 
            $('#payment_type').html(data.payout_type_details.payment_type);
			$('#payout_type_id', F).val(data.payout_type_details.payout_type_id);
            $('#payout_type_key', F).val(data.payout_type_details.payout_type_key);
            ...
            $.each(data.payout_type_details.currency_allowed, function (k, e) {
                options.push($('<option>', {value: e.currency_id}).text(e.currency_code));
            });		
            ...
            $.each(data.breakdowns, function (k, e) {
                breakdowns.push($('<div>', {class: 'row'}).append([
                    ...
                    $('<div>', {class: 'col-sm-2'}).html([e.currency_symbol, ' ', e.currency_code]),
                    $('<div>', {class: 'col-sm-2'}).html([e.currency_symbol, ' ', e.current_balance, ' ', e.currency_code]),
            ...
            $('.'+data.payout_type_details.payout_type_id+'.'+data.currency_code, $('#account-details')).show();
    ...
    F.on('change', '#currency_id,#amount', function () { //alert("fdg");return false;
